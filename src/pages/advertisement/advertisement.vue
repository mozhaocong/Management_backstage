...
export default {
  ...
  data() {
    return {
      ...
      tableHeader: [{ type: 'selection', width: '50' }, { prop: 'shopName', label: '排序', width: 'auto' }, { prop: 'type', label: '分类', width: '100' },
                    { prop: 'imgUrl', label: '图片', width: 'auto' }, { prop: 'imgUrl', label: '链接地址', width: 'auto' }, { prop: 'createTime', label: '加入时间', width: 'auto' },
  ...
  methods: {
    getTableList() {
      this.$https.get("/system/Carousel/getList").then((res) => {
        ...
        this.pagination.total = res.data.data.lenght;
      })
    },
    //leftTree
    leftTreePick(val) {
